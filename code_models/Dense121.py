...
class DenseNet:
    ...
    def __init__(self, num_classes, img_size, channels, weights='imagenet', name="DenseNet", include_top=False):
        self.name = name
        ...
        if self.include_top:
            if self.input_width_height != 224 or self.channels != 3:
                ...
                if self.name == "DenseNet" or self.name == "DenseNet121":
                    base_model = Dense121(weights=self.weights, include_top=False, classes=self.num_classes)
                else:
